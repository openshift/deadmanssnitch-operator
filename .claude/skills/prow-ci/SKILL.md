...
```bash
# View recent PR jobs
curl -s "https://prow.ci.openshift.org/?repo=openshift%2Fdeadmanssnitch-operator&type=presubmit" | grep -E "pull-ci-openshift-deadmanssnitch-operator"
...
- **Pull request jobs**: `gs://test-platform-results/pr-logs/pull/openshift_deadmanssnitch_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]`
- **Periodic jobs**: `gs://test-platform-results/logs/[JOB_NAME]/[JOB_ID]`

**Viewing logs via web**:
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_deadmanssnitch_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]
```
...
- `pull-ci-openshift-deadmanssnitch-operator-master-e2e-binary-build-success` - E2E binary build verification
- `pull-ci-openshift-deadmanssnitch-operator-master-coverage` - Code coverage analysis (with Codecov)
...
- `deadmanssnitch-operator-pull-request` - Main PR pipeline (docker build with OCI-TA)
- `deadmanssnitch-operator-e2e-pull-request` - E2E testing pipeline
- `deadmanssnitch-operator-pko-pull-request` - PKO (Package Operator) pipeline
...
```text
https://prow.ci.openshift.org/?repo=openshift%2Fdeadmanssnitch-operator&type=presubmit
```
...
- Configuration: `ci-operator/config/openshift/deadmanssnitch-operator/openshift-deadmanssnitch-operator-master.yaml`
- Runs: lint, test, validate, coverage, e2e-binary-build
- Uses Codecov for coverage reporting (secret: `deadmanssnitch-operator-codecov-token`)
...
- Builds container images to Quay (deadmanssnitch-operator-tenant)
...
# Option 2: Use gh CLI to find URLs
gh pr checks <PR_NUMBER> | grep "pull-ci-openshift-deadmanssnitch-operator"
...
# https://prow.ci.openshift.org/?repo=openshift%2Fdeadmanssnitch-operator

# Or use platform-specific command:
# macOS: open "https://prow.ci.openshift.org/?repo=openshift%2Fdeadmanssnitch-operator"
# Linux: xdg-open "https://prow.ci.openshift.org/?repo=openshift%2Fdeadmanssnitch-operator"
# Windows: start "https://prow.ci.openshift.org/?repo=openshift%2Fdeadmanssnitch-operator"

# View specific PR on Prow (replace <PR_NUMBER>)
# https://prow.ci.openshift.org/?repo=openshift%2Fdeadmanssnitch-operator&type=presubmit&pull=<PR_NUMBER>
```
...
- Common issues:
  ...
  - Auth failures → Secret configuration in `deadmanssnitch-operator-tenant` namespace
- Local validation:
  ```bash
...
- Location: `ci-operator/config/openshift/deadmanssnitch-operator/openshift-deadmanssnitch-operator-master.yaml`
- Update process: Submit PR to openshift/release repository
- Auto-generated jobs in: `ci-operator/jobs/openshift/deadmanssnitch-operator/`

**Tekton Pipelines** (in this repo):
- Location: `.tekton/` directory
- Files:
  - `deadmanssnitch-operator-pull-request.yaml` - Main PR pipeline
  - `deadmanssnitch-operator-push.yaml` - Post-merge pipeline
  - `deadmanssnitch-operator-e2e-pull-request.yaml` - E2E testing
  - `deadmanssnitch-operator-pko-pull-request.yaml` - PKO validation
...
- Secret: `deadmanssnitch-operator-codecov-token` (stored in Prow)
- Generate coverage locally: `make coverage`
- Coverage runs on PRs and post-merge (`publish-coverage`)
- Dashboard: Check Codecov for deadmanssnitch-operator
